        ...
        <div className="p-8 pt-4 space-y-6">
          ...
          <div className="space-y-5 py-2">
            {[
              {
                title: "데이터 저장",
                desc: "사용자가 입력한 이름, TMI, 이미지는 오직 사용자의 기기에만 저장되며, 개발자는 이를 수집하거나 열람할 수 없습니다."
              },
              {
                title: "AI 처리",
                desc: "모든 대화 생성은 Google Gemini API를 통해 이루어집니다. 입력하신 내용은 AI 답변 생성을 위해 Google로 전달되나, 이는 서비스 제공 목적 이외에 사용되지 않습니다. (단, Google의 약관에 따름)"
              },
              {
                title: "학습 미이용",
                desc: "본 서비스는 유저의 사적인 데이터를 별도로 수집하여 AI를 재학습시키지 않습니다."
              }
            ].map((item, idx) => (
              <div key={idx} className="flex gap-4 items-start">
                ...
                <div className="space-y-1">
                  <h4 className={`text-xs font-black ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>{idx + 1}. {item.title}</h4>
