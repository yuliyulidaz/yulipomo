    ...
    <div className="relative w-full h-screen flex flex-col bg-background text-text-primary overflow-hidden font-sans select-none">
      ...
      <div className="fixed top-0 left-0 w-full z-50">
        ...
        <div className="flex justify-center -mt-0.5">
          <div className="bg-surface px-4 py-1.5 rounded-b-2xl border border-border border-t-0 shadow-sm flex items-center gap-1.5">
      ...
      <main className="flex-1 w-full flex flex-col items-center justify-center p-6 relative z-10 gap-8 md:gap-12 mt-10">
          
          {/* 2. 중앙: 캐릭터 존 (캐릭터 + 하단 대사 상자) */}
          <div className="flex flex-col items-center gap-6 w-full max-w-sm">
            <div className="relative group">
                {shouldHideCharacter ? (
                  <div className="w-48 h-48 md:w-56 md:h-56 flex flex-col items-center justify-center gap-4 animate-pulse text-primary-light/30 bg-slate-50 rounded-full border-2 border-dashed border-border/50">
                    <Bed size={64} />
            ...
            {/* 대사 상자 (세련된 네모 형태) */}
            <div className={`h-24 w-full flex items-center justify-center px-4 transition-all duration-500 ${message ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                {message && (
                  <div className="w-full bg-surface/80 backdrop-blur-md border border-border p-5 rounded-2xl shadow-sm text-center">
            ...
            <div className="text-center space-y-1">
                ...
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/5 rounded-lg">
          ...
          {/* 4. 최하단: 정보 및 컨트롤 */}
          <div className="w-full max-w-[340px] flex flex-col items-center gap-6 animate-in slide-in-from-bottom-8 duration-700">
            
            {/* (상단) Focus Mode 태그 + Cycle 점 4개 */}
            <div className="w-full flex items-center justify-between px-2">
                <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border ${isBreak ? 'bg-success/5 border-success/10 text-success' : 'bg-primary/5 border-primary/10 text-primary'}`}>
                ...
                <div className="flex gap-1.5">
                  {[1, 2, 3, 4].map((i) => (
                    <div 
                      key={i} 
                      className={`w-2 h-2 rounded-full transition-all duration-500 ${i <= sessionInCycle ? 'bg-primary scale-125' : 'bg-border/60'}`} 
                    />
                  ))}
                </div>
            </div>
            ...
            <div className="w-full flex items-center justify-center gap-6">
                ...
                <button 
                    ...
                    className={`h-16 ${isBreak ? 'w-full' : 'w-20'} rounded-3xl bg-surface hover:bg-border flex items-center justify-center transition-all border border-border text-text-secondary hover:text-text-primary active:scale-95 shadow-sm`}
                >
