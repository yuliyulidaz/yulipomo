      ...
      <main className="w-full h-full flex flex-col items-center justify-center relative p-4 md:p-8">
          ...
          <div className={`w-full max-w-md backdrop-blur-xl border p-6 md:p-8 rounded-[40px] shadow-[0_20px_50px_rgba(74,95,122,0.1)] flex flex-col items-center gap-6 md:gap-8 animate-in fade-in zoom-in duration-500 relative transition-colors duration-700 ${isDarkMode ? 'bg-[#161B22]/90 border-[#30363D]' : 'bg-surface/90 border-border'} ${isApiKeyInputVisible || isSettingsOpen ? 'overflow-visible z-40' : 'overflow-hidden'}`}>
            ...
            <div className={`relative mt-9 md:mt-11 min-h-[180px] md:min-h-[220px] flex items-center justify-center w-full transition-all ${isApiKeyInputVisible ? 'z-50' : 'z-20'}`}>
                ...
                {/* 개선된 API 키 변경 팝업 (디테일 강화 버전) */}
                {isApiKeyInputVisible && (
                   <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-[480px] z-[120] animate-in fade-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                      <div className={`p-10 md:p-12 rounded-[3.5rem] shadow-[0_30px_90px_rgba(0,0,0,0.22)] border-[1px] transition-colors ${isDarkMode ? 'bg-[#1B212E] border-white/10' : 'bg-white border-[#F0F2F5]'}`}>
                         ...
                         <div className="relative mb-10">
                            <input 
                               ...
                               className={`w-full h-20 pl-10 pr-16 rounded-[1.8rem] border-[1px] outline-none font-mono text-[16px] transition-all ${isDarkMode ? 'bg-[#0F1219] border-white/5 focus:border-primary text-slate-200' : 'bg-[#F9FAFB] border-[#E2E8F0] focus:border-[#4A5F7A] text-[#2D3436]'}`}
                            />
                            {tempApiKey && (
                         ...
                         {isApiKeyExpiredMode && (
                            <div className="space-y-2.5 mb-10 px-2">
                         ...
                         <button 
                            ...
                            className={`w-full h-20 rounded-[1.8rem] text-[16px] font-black tracking-widest text-white shadow-2xl transition-all active:scale-[0.98] disabled:opacity-30 ${isDarkMode ? 'bg-primary shadow-primary/20' : 'bg-[#4A5F7A] shadow-[#4A5F7A]/30'}`}
                         >
