...
export const PersonalityQuiz: React.FC<PersonalityQuizProps> = ({ 
  ...
  return (
    <div 
      className="flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500 pt-[20vh] cursor-default min-h-full"
      onClick={() => onTempSelect('')}
    >
      {/* 상단 영역: 캐릭터 이미지 아이콘 (간격을 mb-12로 대폭 확대) */}
      <div className="flex-shrink-0 flex justify-center mb-12">
        <div className="w-24 h-24 md:w-28 md:h-28 rounded-full overflow-hidden border-2 border-primary/20 shadow-inner bg-background">
          <img src={imageSrc || ''} className="w-full h-full object-cover" alt="Character Icon" />
        </div>
      </div>

      {/* 상황 설명 문구 (간격을 mb-12로 대폭 확대) */}
      ...
      {/* 캐릭터 대사 리스트 (버튼 사이 간격을 space-y-6으로 확대) */}
        ...
        {options.map((option, index) => {
          ...
          return (
            <button 
              ...
              className={`w-full py-5 px-6 rounded-2xl border-2 transition-all duration-300 relative active:scale-[0.98] flex flex-col items-center justify-center text-center
                ${isSelected 
                  ? 'bg-primary/10 border-primary shadow-md text-primary scale-[1.02]' 
                  : (tempSelection ? 'bg-transparent border-transparent text-slate-200 scale-[0.97]' : 'bg-transparent border-transparent text-text-secondary hover:text-primary')
                }`}
            >
