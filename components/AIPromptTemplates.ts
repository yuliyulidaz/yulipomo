...
export const LEVEL_ACTING_GUIDE: Record<number, LevelActingGuide> = {
  ...
  4: { label: '편한 동료', description: '거리감이 완화되어 말투가 부드러운 상태.', allowed: ['가벼운 온기', '완곡한 반응'], forbidden: ['의존 표현'], example: '생각보다 안정적이네.' },
...
export const buildRefillPrompt = (profile: CharacterProfile, category: string, timeContext: string, seasonContext: string, situations?: ActionSituation[]) => {
  ...
  const count = isClick ? 3 : 5;
  
  let missionHeader = isClick 
    ? `유저가 당신을 클릭했습니다. 아래 제시된 ${count}가지 [Action] 지문을 읽고, 당신의 성격과 유저와의 관계(Lv.${profile.level})에 맞춰 각각 한 줄씩의 '리액션 대사'를 작성하세요.`
    : `유저가 딴짓을 하거나 주의가 산만해진 상황입니다. 유저를 꾸짖거나 주의를 환기시키는 대사 ${count}개를 작성하세요.`;
...
4. [포맷]: 따옴표 없이 문장만 줄바꿈으로 구분하여 총 ${count}줄 출력.
`;
};
