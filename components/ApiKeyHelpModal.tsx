          ...
          <div className="space-y-6">
            <div className="space-y-3">
              ...
              <p className={`text-[11px] font-medium leading-relaxed px-1 ${isDarkMode ? 'text-slate-400' : 'text-text-secondary'}`}>
                최애 캐릭터가 당신의 행동(집중, 딴짓, 클릭 등)을 실시간으로 인지하고, 상황에 맞는 생생한 대화와 관찰 일지를 작성하기 위해 Google의 고성능 AI인 Gemini 모델을 사용합니다. 사용자 개별 API 키를 사용함으로써 더 빠르고 자유로운 대화 경험을 제공할 수 있습니다.
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 pl-1">
                {[
                  ...
                  "방금 만든 프로젝트 선택",
                  "키 만들기 클릭",
                  "가장 앞 '키' 하단에 있는 파란 글씨 클릭",
                  "API 세부정보에서 가장 위에 있는 정보 복사 (Alza로 시작)",
                  "본 페이지로 돌아와 붙여넣기"
                ].map((step, idx) => (
                  <div key={idx} className="flex gap-3 items-start group">
                    <div className={`shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center text-[10px] font-black transition-colors ${isDarkMode ? 'border-slate-700 text-slate-500' : 'border-slate-200 text-slate-400'}`}>
                      {idx + 1}
                    </div>
                    <p className={`text-[11px] font-medium leading-relaxed pt-0.5 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                      {step}
                    </p>
                  </div>
                ))}
